...
- If using a certificate chain, remove the end-entity
- Enter the certificate chain with the Root at the bottom.
...
def load_certificate(certificate, is_file):
    """
    Loads the PEM or DER certificate into a certificate object. 
    Can load files (is_file=True) or from a string (is_file=False)
    """
            ...
            try:
                with open(certificate, "rb") as certificate_file:
                    certificate = x509.load_pem_x509_certificate(
                        certificate_file.read(), default_backend())
                    return certificate
                ...
                except Exception as pem_der_exception:
...
def load_certificate_chain(certificate_chain):
    ...
    Only supports PEM at the moment.
    """
...
def get_ski_aki(certificate):
    """
    Gets the Subject and Authority key identifier of the certificate
    """
    ...
    try:
        aku_value = hex(int.from_bytes(
            certificate.extensions.get_extension_for_class(
                x509.AuthorityKeyIdentifier).value.key_identifier, byteorder="big")).removeprefix("0x").upper()
...
def certificate_chain_verify_workflow(end_entity, certificate_chain_obj_list):
    """
    Verifies the end-entity with the certificate chain
...
def main():
    ...
    argparse_main.add_argument(
        "-e", "--end-entity", nargs="?", required=True, help="Define your end-entity certificate (PEM/DER) file to verify")
    argparse_main.add_argument(
        "-i", "--issuers", nargs="?", required=True, help="Define the end-entity's CA certificate issuer(s) file. Accepts a full certificate chain or single issuer file. Must be in PEM"
    )
    argparse_main.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="Enable verbosity (more wordiness)")
    ...
    if multiple_issuers:
        ...
        chain_verified_instances = certificate_chain_verify_workflow(
            end_entity=end_entity,
            certificate_chain_obj_list=certificate_chain_obj_list)
